...
import { ScanFace, X, CheckCircle, ShieldCheck } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import dynamic from "next/dynamic";

// Dynamically import FaceLivenessDetector to avoid SSR issues with AWS Amplify
const FaceLiveness = dynamic(() => import("./FaceLivenessDetector"), {
  ssr: false,
  loading: () => (
...
function generateVerifiedPlaceholder(): File {
  ...
  gradient.addColorStop(0, "#25B181");
  gradient.addColorStop(1, "#1a8a63");
...
export default function SelfieCapture({
  ...
  return (
    <AnimatePresence>
      {/* Full-screen overlay — edge-to-edge on mobile */}
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 sm:py-3.5 bg-gradient-to-r from-[#25B181] to-[#51C9AF] flex-shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-base sm:text-lg font-bold text-white leading-tight">Face Verification</h2>
                <p className="text-[10px] sm:text-xs text-white/80">3D liveness check</p>
              </div>
            </div>
            <button
              onClick={handleClose}
              className="min-w-[40px] min-h-[40px] flex items-center justify-center text-white hover:bg-white/20 rounded-full transition-colors active:scale-95 touch-manipulation"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain">
            {verified ? (
              ...
              >
                ...
                <p className="text-gray-500 text-xs sm:text-sm">Liveness check passed successfully</p>
              ...
              <div className="p-3 sm:p-4">
                {/* Compact instruction — single line on mobile */}
