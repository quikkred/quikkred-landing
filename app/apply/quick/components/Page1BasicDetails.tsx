...
import { Phone, IndianRupee, CheckCircle, AlertCircle, Zap, Loader2 } from 'lucide-react';
...
export default function Page1BasicDetails({ formData, setFormData, onNext }: Page1Props) {
        ...
        const fetchProducts = async () => {
            ...
            try {
                ...
                if (data.success && data.data) {
                    ...
                    if (!formData.productId && data.data.length > 0) {
                        handleProductChange(data.data[0]);
                    }
                }
            } catch (error) {
    ...
    const handleProductChange = (product: LoanProduct) => {
        setSelectedProduct(product);
        const purpose = `${product.productName} - ${product.category}`;
        const defaultTenure = product.allowedTenures?.length > 0
            ? product.allowedTenures[0]
            : product.minTenure || LOAN_CONFIG.DEFAULT_TENURE;
        setFormData(prev => ({
            ...prev,
    ...
    // Tenure options: from selected product or fallback to config
    const tenureOptions = selectedProduct?.allowedTenures?.length
        ? selectedProduct.allowedTenures
        : (LOAN_CONFIG.TENURE_OPTIONS as unknown as number[]);
    ...
    // Use product-specific rates if available, otherwise use defaults
    const loanCalc = useMemo(() => {
        if (selectedProduct) {
            const amount = formData.loanAmount;
            const tenure = formData.tenure;
            const processingFeePercent = selectedProduct.processingFee || LOAN_CONFIG.PROCESSING_FEE_PERCENT;
            const gstPercent = selectedProduct.gst || LOAN_CONFIG.GST_PERCENT;
            const dailyRate = selectedProduct.dailyInterestRate || LOAN_CONFIG.INTEREST_RATE_PER_DAY;

            const processingFee = Math.round((amount * processingFeePercent) / 100);
            const gstOnProcessingFee = Math.round((processingFee * gstPercent) / 100);
            const totalDeductions = processingFee + gstOnProcessingFee;
            const netDisbursalAmount = amount - totalDeductions;
            const totalInterest = Math.round((dailyRate / 100) * tenure * amount);
            const totalRepayment = amount;

            return {
                loanAmount: amount, tenure, interestRate: dailyRate,
                processingFeePercent, gstPercent,
                processingFee, gstOnProcessingFee, totalDeductions,
                netDisbursalAmount, totalInterest, totalRepayment,
                dueDate: new Date(Date.now() + tenure * 86400000).toISOString().split('T')[0],
            };
        }
        return calculateLoanDetails(formData.loanAmount, formData.tenure);
        ...
        >
                    ...
                    <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
                        {loanProducts.map((product) => (
                            <button
                                ...
                                className={`py-2 sm:py-2.5 px-2 rounded-lg font-medium text-[10px] sm:text-xs transition-all ${formData.productId === product._id
            ...
            <div>
                ...
                {loadingProducts ? (
                    ...
                    <div className={`grid gap-1.5 sm:gap-2 ${tenureOptions.length <= 3 ? 'grid-cols-3' : tenureOptions.length <= 5 ? 'grid-cols-5' : 'grid-cols-4'}`}>
