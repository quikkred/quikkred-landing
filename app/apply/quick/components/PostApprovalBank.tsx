...
export default function PostApprovalBank({
    ...
    // Penny Drop Verification
    const handlePennyDrop = async (): Promise<boolean> => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                ...
                setErrors({ submit: data.message || 'Bank verification failed. Please check your details.' });
                return false;
            }
        } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        if (MOCK_MODE) {
            ...
            trackStepCompleted(4, 'Bank Details', { bankName: formData.bankName });
        ...
        try {
            // Step 1: Penny drop verification (if not already verified)
            if (!bankVerified) {
                const pennyDropOk = await handlePennyDrop();
                if (!pennyDropOk) {
                    trackBankVerifyFailed('Penny drop failed', bankFriction.getAttempts());
                    setSubmitLoading(false);
                    return;
                }
            }
    ...
    return (
        ...
        >
                ...
                <div className="space-y-3">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className={`w-full px-3 sm:px-4 py-2.5 sm:py-3 border rounded-lg text-sm focus:ring-2 focus:ring-[#25B181] uppercase ${bankVerified ? 'bg-gray-100 text-gray-600' : ''} ${errors.ifsc || ifscError ? 'border-red-500' : 'border-gray-300'
                                        }`}
                                />
                                {ifscLoading && (
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={bankVerified}
                            className={`w-full px-3 sm:px-4 py-2.5 sm:py-3 border rounded-lg text-sm focus:ring-2 focus:ring-[#25B181] ${bankVerified ? 'bg-gray-100 text-gray-600' : ''} ${errors.accountNumber ? 'border-red-500' : 'border-gray-300'
                                }`}
                        />
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={bankVerified}
                            className={`w-full px-3 sm:px-4 py-2.5 sm:py-3 border rounded-lg text-sm focus:ring-2 focus:ring-[#25B181] ${bankVerified ? 'bg-gray-100 text-gray-600' : ''} ${errors.accountHolderName ? 'border-red-500' : 'border-gray-300'
                                }`}
                        />
                    ...
                    {/* Penny Drop Status */}
                    {bankVerified ? (
                        <div className="bg-green-50 rounded-lg p-2.5 sm:p-3 flex items-center gap-2 border border-green-200">
                            <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0" />
                            <p className="text-[10px] sm:text-xs text-green-700 font-medium">
                                Bank account verified successfully via penny drop
                            </p>
                        </div>
                    ) : bankVerifying ? (
                        <div className="bg-blue-50 rounded-lg p-2.5 sm:p-3 flex items-center gap-2 border border-blue-200">
            ...
            <p className="text-center text-[10px] sm:text-xs text-gray-500">
                Step 1 of 5: <span className="font-medium text-[#25B181]">Bank</span> → Aadhaar → Selfie → References → BSA
            </p>
        </motion.div>
    );
}
