...
import Page4Approval from "../Page4Approval";
...
const FormSteps = ({
    ...
    const currentStep = useMemo(() => (step === "bank" ? 2 : step === "mandate" ? 3 : 1), [step]);

    const handleChangeStep = (nextStep: FormStepsType) => {
        if (nextStep === "eligibility") {
            tracking.trackEvent('STEP_COMPLETED', { stepNumber: 1, stepName: 'Check Eligibility' });
            ...
            tracking.trackEvent('STEP_COMPLETED', { stepNumber: 2, stepName: 'Bank Verification' });
        }
        setStep(nextStep);
    ...
    if (!isLogin) {
        return <main className="flex-1 w-full max-w-lg mx-auto px-3 py-3 sm:py-6">
            <CustomerLogin />
        </main>
    }
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            {step === 'bank' && (
                                <BankVerification
                                    ...
                                    formData={formData}
                                    setFormData={setFormData}
                                    onNext={() => handleChangeStep("mandate")}
