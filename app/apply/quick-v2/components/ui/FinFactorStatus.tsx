...
import { useRouter } from "next/navigation"; // or "nextjs-toploader/app" if using that
...
interface FinFactorStatusProps {
  ...
  loading: boolean;
  data: BreData | null;
  onContinue?: () => void; // Optional callback if you want parent to handle next step
}
...
const FinFactorStatus = ({ visibility, loading, data, onContinue }: FinFactorStatusProps) => {
  ...
  const isRejected = data?.status?.toLowerCase() === "reject";
  ...
  const handleContinue = () => {
    if (onContinue) {
      ...
      // Default behavior: Go to next verification step
      // REPLACE THIS with your actual next route
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden ring-1 ring-black/5"
        >
          {/* ---------------- LOADING STATE ---------------- */}
          {loading && (
