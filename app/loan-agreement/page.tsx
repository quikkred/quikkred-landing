...
const FALLBACK_LOAN_NUMBER = 'LN20261770652535715';

export default function LoanAgreementPage() {
  const searchParams = useSearchParams();
  const LOAN_NUMBER = searchParams.get('loanNumber') || FALLBACK_LOAN_NUMBER;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!token) {
        setError('Please login first. No auth token found.');
        return;
      }
      ...
      try {
        // Fetch customer data
        const customerResponse = await fetch(`${API_BASE_URL}/api/customer/get`, {
          method: 'GET',
          headers: {
        ...
        // Fetch loan data
        const loanResponse = await fetch(`${API_BASE_URL}/api/loans/get/${LOAN_NUMBER}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        if (loanResponse.ok && loanResult.success && loanResult.data) {
  ...
  const generateAgreementHTML = (data: any) => {
    ...
    const today = data.disbursementDate ? new Date(data.disbursementDate) : new Date();
    ...
    const documentNumber = `DOC${new Date().getFullYear()}${Date.now()}`;
  ...
  return d.toLocaleDateString('en-IN', {
    ...
    year: 'numeric'
      ...
      if (tenure <= 45) {
        // Use actual due date from loan data, fallback to calculated
        let dueDate: Date;
        if (data.firstDueDate) {
          dueDate = new Date(data.firstDueDate);
        } else {
          dueDate = new Date();
      ...
      for (let i = 1; i <= months; i++) {
        // Use actual first due date from loan data as base, fallback to calculated
        let dueDate: Date;
        if (data.firstDueDate) {
          dueDate = new Date(data.firstDueDate);
...
<div class="details">
    ...
    <div class="esign-box lender-box">
        ...
        <div class="approve-section" id="approve-section">
            ...
            <div style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                <button class="download-btn" id="download-btn" onclick="testGeneratePDF()">&#128229; Download PDF</button>
                <button class="approve-btn" id="approve-btn" onclick="approveAgreement()">I Agree & Approve</button>
            </div>
            <p style="font-size: 11px; color: #666; margin-top: 15px;">Download PDF saves a copy locally. Approve uploads to server and proceeds with eSign.</p>
    ...
    <script>
        ...
        async function testGeneratePDF() {
            ...
            try {
                // Hide approve section temporarily
                ...
                // Generate clean PDF using html2pdf (no browser headers/footers)
                await html2pdf().set({
                    margin: 10,
                    filename: 'Quikkred-Loan-Agreement.pdf',
...
y += boxHeight + 4;
            ...
            const currentDate = '${data.disbursementDate ? new Date(data.disbursementDate).toLocaleDateString("en-IN") : new Date().toLocaleDateString("en-IN")}';
            const currentTime = '${data.disbursementDate ? new Date(data.disbursementDate).toLocaleTimeString("en-IN") : new Date().toLocaleTimeString("en-IN")}';
    ...
    try {
        ...
        const response = await fetch('${API_BASE_URL}/api/kyc/eSign/upload?documentNumber=' + documentNumber, {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + token
            },
  ...
  const handleGenerateAgreement = async () => {
    ...
    // Build agreement data from customer + loan data
    ...
    const agreementData = {
      ...
      address: data.currentAddress?.fullAddress || '',
      ...
      ifscCode: data.banks?.[0]?.ifscCode || '',
      accountHolderName: data.banks?.[0]?.accountHolderName || data.fullName || '',
      loanAmount: loan.loanAmount || loan.principalAmount || '',
      tenure: loan.tenure || '',
      tenureUnit: loan.tenureUnit || 'Days',
      productName: loan.productName || 'Personal Loan',
      interestRate: loan.interestRate || loan.dailyInterestRate || '',
      processingFee: loan.processingFee || '',
      totalInterest: loan.totalInterest || '',
      gstOnProcessingFee: loan.gstOnProcessingFee || '',
      totalAmount: loan.totalRepayment || loan.totalAmount || '',
      disbursementAmount: loan.netDisbursalAmount || loan.disbursementAmount || '',
      applicationNumber: loan.loanNumber || LOAN_NUMBER,
      firstDueDate: loan.firstDueDate || loan.nextDueDate || '',
      maturityDate: loan.maturityDate || '',
      disbursementDate: loan.disbursementDate || '',
    };

    // Store token in localStorage so the agreement page can access it
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: 'white', borderRadius: '16px', padding: '40px', maxWidth: '500px', width: '100%', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
        <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#1a202c', marginBottom: '8px', textAlign: 'center' }}>
          Loan Agreement
        </h1>
        <p style={{ color: '#718096', textAlign: 'center', marginBottom: '24px', fontSize: '14px' }}>
          Loan Number: <strong style={{ color: '#25B181' }}>{LOAN_NUMBER}</strong>
        </p>

        {error && (
          <div style={{ background: '#FEE2E2', border: '1px solid #EF4444', borderRadius: '8px', padding: '12px', marginBottom: '16px', color: '#B91C1C', fontSize: '14px' }}>
            {error}
          </div>
        )}
        ...
        {dataLoaded && (
          <div style={{ marginBottom: '24px' }}>
            <div style={{ background: '#F0FDF4', borderRadius: '8px', padding: '16px', marginBottom: '12px' }}>
              <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#166534', marginBottom: '8px' }}>Customer Data</h3>
              <p style={{ fontSize: '13px', color: '#4a5568' }}>Name: {customerData?.fullName || 'N/A'}</p>
              <p style={{ fontSize: '13px', color: '#4a5568' }}>Mobile: {customerData?.mobile || 'N/A'}</p>
              <p style={{ fontSize: '13px', color: '#4a5568' }}>Email: {customerData?.email || 'N/A'}</p>
            </div>

            <div style={{ background: '#EFF6FF', borderRadius: '8px', padding: '16px' }}>
              <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#1E40AF', marginBottom: '8px' }}>Loan Data</h3>
              <p style={{ fontSize: '13px', color: '#4a5568' }}>Loan Amount: {loanData?.loanAmount || loanData?.principalAmount || 'N/A'}</p>
              <p style={{ fontSize: '13px', color: '#4a5568' }}>Tenure: {loanData?.tenure || 'N/A'} {loanData?.tenureUnit || 'Days'}</p>
              <p style={{ fontSize: '13px', color: '#4a5568' }}>Product: {loanData?.productName || 'N/A'}</p>
            </div>
          </div>
        )}
        ...
        <button
          ...
          style={{
            width: '100%',
            padding: '16px',
            background: loading || !dataLoaded ? '#CBD5E0' : 'linear-gradient(135deg, #25B181, #51C9AF)',
            color: 'white',
            border: 'none',
            borderRadius: '12px',
            fontSize: '16px',
            ...
            boxShadow: '0 4px 12px rgba(37, 177, 129, 0.3)',
          }}
        >
          {loading ? 'Loading...' : 'Generate & Sign Loan Agreement'}
        </button>

        <p style={{ textAlign: 'center', marginTop: '16px', fontSize: '12px', color: '#A0AEC0' }}>
          This will open the agreement in a new tab for e-Sign
        </p>
      </div>
    </div>
  );
