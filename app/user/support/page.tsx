...
export default function SupportPage() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      const response = await fetch('http://93.127.167.88:4050/api/supportTicket/getAll', {
        method: 'GET',
        headers: {
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://93.127.167.88:4050/api/supportTicket/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
